...
extern "C" void hash3_vector(std::vector<uint32_t>& freq, const std::string& input_file, 
      ...
      while (true) {
        ...
        if (j < N) {
          ...
          for (size_t k = 0; k < leftover && is_eq; ++k) {
            if (data_acc[i + cycles * VEC_LEN + k] != data_acc[(j - len_ + 1) + cycles * VEC_LEN + k]) {
              is_eq = false;
            }
          }
          if (is_eq) ++res;
